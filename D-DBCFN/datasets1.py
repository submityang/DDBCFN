...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG, patch_size=5):
    ...
    folder = target_folder + datasets[dataset_name].get('folder',
                                                        dataset_name + '/')  # get()函数返回指定键的值，如果指定键的值不存在时，返回该默认值值。
    ...
    if dataset_name == 'PaviaC':
        ...
        # img1 = np.resize(img[:, :, 55], (610, 340, 1))
        # img2 = np.resize(img[:, :, 41], (610, 340, 1))
        # img3 = np.resize(img[:, :, 12], (610, 340, 1))
    ...
    ignored_labels = list(
        set(ignored_labels))  # set(list_a) 将list_a中的出现过的数据做成一个集合 如list = [1, 1, 3, 2, 3] --> {1, 2, 3}
    # Normalization
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, **hyperparams):
        ...
        for item_x, item_y in temp_indices:
            ...
            self.indice = []
            for loc in self.indices:
                if [loc[0], loc[1]] not in self.indice:
                    self.indice.append([loc[0], loc[1]])
